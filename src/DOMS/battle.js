...
const battle = (() => {
  ...
  const displayPlayerMessage = (boardElement, ship = false) => {
    ...
    if (boardElement !== "x") {
      if (ship && !ship.isSunk) {
        ...
        displayMessage(
          friend,
          messages.getEnemySunkMessage(friend.textContent),
        );
      } else {
        displayMessage(
          friend,
          messages.getPlayerMissMessage(friend.textContent),
        );
      }
    }
  ...
  const playerHits = (info) => {
    ...
    if (info.battleship.isSunk) {
      ...
      fleet.loadShipsOnBoard(info.cpu, {
        map,
        board: enemyBoard,
        boardElement: board[info.row][info.col],
        i,
        j,
      });
    }
